...
const OilPriceChart = () => {
  ...
  const [traceData, setTraceData] = useState([
    { key: 'brent', min: 400, max: 600, lastDate: '' },
    { key: 'wti', min: 80, max: 120, lastDate: '' },
  ])
  ...
  useEffect(() => console.log('trace:', layout), [layout])

  useEffect(() => {
    const setLayoutOnChart = (item: any) => {
        ...
        shapes: [
          {
            type: 'line',
            x0: verticalLineX,
            y0: traceData.find((x) => x.key === item).min,
            x1: verticalLineX,
            y1: traceData.find((x) => x.key === item).max,
            line: {
              color: 'darkgrey',
              width: 1,
              dash: 'dash',
  ...
  const fetchChartData = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        //초기 레이아웃 (brent)
        const settings = [
          { key: 'brent', min: 400, max: 600, lastDate: respData['brent'].index_hist.slice(-1)[0].toString() },
          { key: 'wti', min: 80, max: 120, lastDate: respData['wti'].index_hist.slice(-1)[0].toString() },
        ]

        setTraceData(settings)
