...
export const D3LineChartIntervalSmall: React.FC<LineChartPorps> = (props) => {
  const svgRef3 = React.useRef()
  ...
  const DrawD3LineChartInterval = (data: any) => {
    ...
    if (data[0].x !== undefined && data[0].y !== undefined) {
      ...
      data.forEach((d: any) => {
        ...
        console.log(parseTime(new Date(d.x)))
        console.log(parseDate(parseTime(new Date(d.x))))
        d.value = d.y
        delete d.x
        delete d.y
      })
...
export default D3LineChartIntervalSmall
