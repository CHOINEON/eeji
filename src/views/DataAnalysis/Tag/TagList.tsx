...
const TagList = (props: any) => {
  ...
  const handleSave = () => {
    axios
      .post('http://ec2-3-220-205-197.compute-1.amazonaws.com:9871/createTag', JSON.stringify(rowData))
      .then((response) => {
  ...
  const handleDelete = () => {
    ...
    if (selectedRowData) {
      axios
        .delete('http://ec2-3-220-205-197.compute-1.amazonaws.com:9871/deleteTag/' + selectedRowData.id)
        .then((response) => {
          console.log('resp:', response)
  ...
  return (
    <>
      ...
      <Box sx={{ m: 2, height: 550 }} id="targetElement">
        <div className="ag-theme-alpine" style={gridStyle}>
          ...
          {/* <Stack sx={{ mt: 2 }} spacing={1} direction="row" justifyContent="flex-end">
            <Button type="primary" color="success" onClick={handleSave}>
              저장
            </Button>
          </Stack> */}
        </div>
